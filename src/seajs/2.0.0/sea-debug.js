...
function addOnload(node, callback, isCSS) {
  ...
  node.onload = node.onerror = node.onreadystatechange = function() {
    ...
    if (READY_STATE_RE.test(readyState)) {
      ...
      if (readyState == "loaded") {
		// try to fix a weird bug by ralph_lu@qq.com
		// when readyState is "loaded", maybe the code hasn't been executed yet
...
global.define = _define ? function() {
  isFunction(_define) && _define.apply(global, arguments)
