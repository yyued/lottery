...
(function(seajs) {
	...
	var REG_DW_STATIC = /^http:\/\/assets\.dwstatic\.com\/(.+)/i, REG_CSS = /\.css$/i;
	...
	function getCacheSuffix(uri, single) {
		var versions = [data.comboVersion || "0"], moduleVersion = data.moduleVersion;
	...
	function setHash(root, files, dwRoot, commonPath) {
		...
		if (files.length > 1 && exceedMax) {
			var parts = splitFiles(files, comboMaxLength - (comboPath.length - files.join(comboSyntax[1]).length))
...
(function(window) {
	function globalConfig() {
		if (seajs.pluginLoaded.globalConfig) return;
		...
		seajs.pluginLoaded.globalConfig = true;
	}
	...
	var sessionDebug = seajs.sessionDebug = function(base, updateSession) {
		...
		window.console && window.console.log && window.console.log("session debug base:" + base);
